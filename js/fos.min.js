...
class Icon extends HTMLElement {
    ...
    calcPos() {
        ...
        this.top = 8 + 104 * Math.floor(howMany / x) + offset,
        this.left = 8 + howMany % x * 72
    }
    attributeChangedCallback(name, oldValue, newValue) {
        this.render()
    }
...
class Window extends HTMLElement {
    ...
    connectedCallback() {
        ...
        this.top = .2 * innerHeight * howMany / 5,
        this.left = .1 * innerWidth * howMany / 5,
        this.render()
    }
    render() {
        this.shadow.innerHTML = "";
...
class Desktop extends HTMLElement {
    ...
    mouseMove(e) {
        if (this.selected && !this.selected.fixed)
            if (e.preventDefault(),
            "fos-icon" === this.selected.localName) {
                const x = e.pageX || (e.touches ? e.touches[0].pageX : 0)
                  , y = e.pageY || (e.touches ? e.touches[0].pageY : 0);
                this.selected.style.left = 8 * Math.floor(x / 8) - this.selected.offsetWidth / 2 + "px",
                this.selected.style.top = 8 * Math.floor(y / 8) - this.selected.offsetHeight / 2 + "px"
            } else {
...
class Menu extends HTMLElement {
    ...
    connectedCallback() {
        for (this.hasAttribute("tabindex") || this.setAttribute("tabindex", 0),
        this.bar = this.parentNode; "FOS-BAR" !== this.bar.tagName; )
